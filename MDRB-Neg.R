#The negtive ion mode uses the following code
...
write.xlsx(Pro_drug_sample, file = paste(File[3],"_Result.xlsx"),
           sheetName = "Prototypes",append=F)

write.xlsx(Iso_drug_sample, file = paste(File[3],"_Result.xlsx"),
...
One_Route_add[[2]] <- One_Route + 12 + 15.995*2 + 1.008*2 #+(HCOO+H)
One_Route_add[[3]] <- One_Route - 12 - 15.995*2 - 1.008*2 #-(HCOO+H)
...
write.xlsx(One_Route_pro, file = paste(File[3],"_Result.xlsx"),
...
Two_Route_add[[2]] <- Two_Route_met + 12 + 15.995*2 + 1.008*2 #+(HCOO+H)
Two_Route_add[[3]] <- Two_Route_met - 12 - 15.995*2 - 1.008*2 #-(HCOO+H)
...
write.xlsx(Two_Route_pro, file = paste(File[3],"_Result.xlsx"),
...
if(length(Met_drug_screen_1[,1]) != 0){
  ...
  if(nrow(Final_table1)!=0){
    ...
    Met_drug_2nd$Dec2 <- Met_drug_2nd$HRMS..m.z - floor(Met_drug_2nd$HRMS..m.z)#要确保是四位小数
...
write.xlsx(Final_table1, file = paste(File[3],"_Result.xlsx"),
...
if (length(Iso_drug_sample) != 0){
  ...
  write.xlsx(Iso_screen, file = paste(File[3],"_Result.xlsx"),
...
if(nrow(Met_drug_screen_2) != 0){
  ...
  write.xlsx(Met_drug_screen_2, file = paste(File[3],"_Result.xlsx"),
